...
Requirements that are viewed as improvements to the product, but not strictly required for the product to be usable, i.e. the product can still be used without these. These include performance (how fast the page loads, how fast the interaction takes), scalability (how many items can be present on the page), good user experience, etc. Non-functional requirements for e-commerce sit are:

- **SEO:** Check if search engine optimisation is required or not. Use semantic HTML in that case.
- **Performance:** Core Web Vitals, Asset Optimization (CSS, JS, Images)
...
- **Caching:** In local storage / session storage / index db or using Service Workers.
...
- Make sure that for a particular functionality, all network calls are in parallel if they are not dependent on responses of each other.
...
- Limit the number of concurrent API calls. This reduces network congestion and server overload.
...
- Lazy loading / Code splitting: Route based, component level splitting, library splitting.
...
- Asset Optimization: 
    ...
    - Use native lazy loading (`loading="lazy"`) or libraries like `react-lazyload`.
